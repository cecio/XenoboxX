...
//
// Search strings in process memory
//
// compile with:
// cl.exe /O1 /Os /Oy /FD /MT /GS- /J /GR- /FAcs /W4 /Zl /c /TC /kernel wx64_common.c
// cl.exe /O1 /Os /Oy /FD /MT /GS- /J /GR- /FAcs /W4 /Zl /c /TC /kernel wx64_memgrep.c
// ml64.exe wx64_common_a.asm /Fewx64_memgrep.exe /link /NODEFAULTLIB /RELEASE /MACHINE:X64 /entry:main wx64_memgrep.obj wx64_common.obj
// shellcode64.exe -o wx64_memgrep.exe "SEARCH STRINGS IN MEMORY                            \n===============================================================\nREQUIRED OPTIONS:                                              \n  -0   : Process PID to open. Example '-0 0x0fe0'.        \nOPTIONAL OPTIONS:                                              \n  -1   : Process monitoring timeout Default: 0x20. Example:  '-1 0x100'. \n  -2   : ASCII (0x0) or WIDE (0x1) string. Default: 0x0\n  -3   : Case sensitive (0x1) or insensitive (0x0) search. Case sensitive is faster. Default: 0x0\n  -4   : Search only Writable memory pages (0x0) or all (0x01). Default: 0x0\n  -s   : Search string (in doublequotes)\n===== RESULT OF MEMGREP OPERATION ======================%s\nNTSTATUS  : 0x%08X                                             \n===============================================================\n"
//
...
NTSTATUS SearchMem(PKMDDATA pk, HANDLE ProcessHandle, PVOID BaseAddress, SIZE_T RegionSize, BOOLEAN Wide, BOOLEAN CaseSensitive)
{
    ...
    while ((result = KernelGrep(MemBuffer, RegionSize, pk->dataInStr, Wide, CaseSensitive, offset, 1)) != NULL) {

        if (KernelGrep((PVOID) qwAddrOutBase, pk->dataOutExtraLength, result, Wide, CaseSensitive, 0, 0) == NULL) {
...
NTSTATUS QueryAllVirtualMemoryRegions(PKMDDATA pk, HANDLE ProcessHandle, BOOLEAN Wide, BOOLEAN Case, BOOLEAN allPages)
{
    NTSTATUS status = STATUS_SUCCESS;
        ...
        if (NT_SUCCESS(status))
        {
            ...
            if (mbi.Type != MEM_IMAGE) {
                if ( allPages == TRUE || (mbi.Protect & PAGE_READWRITE ||
                    ...
                    SearchMem(pk, ProcessHandle, mbi.BaseAddress, mbi.RegionSize, Wide, Case);
                }
            }
...
VOID c_EntryPoint(_In_ PKMDDATA pk)
{
    ...
    while (TRUE) {
        // print_Iteration(pk);
        QueryAllVirtualMemoryRegions(pk, ZwProcessHandle, isWide, caseSensitive, allPages);
